...
def fine_servo(
    ...
    skip_search: True when the caller already IK-moved to a known-good
    approximate position (click_pick_place.py, from a user click) - skips
    search()'s own blind SEARCH_OFFSETS grid sweep, which real hardware
...
def run(
    arm: SOArm101,
    cap: perception.PublishedFrameSource,
    ...
    grasp_strategy: "legacy" (default, unchanged) runs fine_servo() then
    descend_and_grasp(). "il" replaces exactly that pair with a short ACT
    skill (il_grasp_skill), keeping search()+coarse_center() in front of it
    so the skill always starts from the same standardized pre-grasp hover -
    that start-state standardization is the whole point of the hybrid split.
    policy_path is required only for "il"."""
    if grasp_strategy not in ("legacy", "il"):
        raise ValueError(f"grasp_strategy must be 'legacy' or 'il', got {grasp_strategy!r}")
    if grasp_strategy == "il" and not policy_path:
        raise ValueError("grasp_strategy='il' needs policy_path (a trained ACT checkpoint directory).")
    ...
    try:
        ...
        for attempt in range(1, config.MAX_GRASP_ATTEMPTS + 1):
            ...
            try:
                ...
                # 2026-09-01: this pre-positioning move had no guard, unlike
                # every other move in this file (see search()'s grid-sweep
                # comment) - crashed main.py with a raw traceback on a real
                # run instead of failing this attempt cleanly. search() (called
                # by fine_servo below) retries the same target itself anyway,
                # so there's nothing this needs beyond logging and moving on.
                _log(TaskState.SEARCH, f"시도 {attempt}: 탐색 위치 이동 중 충돌 감지 ({e}) - 그대로 탐색 진행")
            if grasp_strategy == "il":
                # Legacy vision+IK gets the arm to a standardized start state
                # (search() finds the cube, coarse_center() puts it at
                # GRASP_TARGET_PX; both leave z at SEARCH_HOVER_XYZ[2], so the
                # resulting pose IS the pre-grasp hover - no extra IK move
                # needed). Then the ACT skill does the contact part.
                # Imported here, not at module top: it pulls in torch/lerobot
                # policy machinery, which legacy runs (main.py) must not pay for.
                import il_grasp_skill

                if not search(arm, cap, perception.detect_red_cube, "빨간 큐브") or not coarse_center(
                    arm, cap, perception.detect_red_cube
                ):
                    _log(TaskState.FAILED, f"시도 {attempt}: pre-grasp 표준화 실패")
                    continue
                try:
                    ok = il_grasp_skill.run_grasp_skill(arm, cap, policy_path)
                except CollisionDetected as e:
                    # Same convention as fine_servo(), which swallows a collision
                    # and returns False: one blocked attempt is a failed attempt,
                    # not an aborted task - the retry loop below handles it.
                    _log(TaskState.FAILED, f"시도 {attempt}: IL 스킬 중 충돌 감지 ({e})")
                    ok = False
            else:
                if not fine_servo(arm, cap, perception.detect_red_cube, "빨간 큐브"):
                    _log(TaskState.FAILED, f"시도 {attempt}: 큐브 정렬 실패")
                    continue
                ok = descend_and_grasp(arm)
            if ok:
                grasped = True
                break
