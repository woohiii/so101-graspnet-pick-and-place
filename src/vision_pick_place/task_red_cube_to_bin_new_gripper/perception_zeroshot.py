...
# 2026-09-01: added as a second backend option per the user's request to try
# NVIDIA's hosted API catalog (build.nvidia.com/NIM) - free-tier credits
# (no card required), NOT unlimited (~1000 credits/account, 40 req/min cap -
# verified against NVIDIA's own docs/forums, contra the user's "다 무료야").
# 2026-09-01: model hunt against this account's real /v1/models + real calls
# (deepseek-v4-pro is text-only; deepseek-v4-flash-vision-exp/deepseek-v4-
# flash aren't hosted/are EOL; llama-3.2-11b responded but returned a
# degenerate center-point box - didn't actually localize anything; llama-
# 3.2-90b timed out repeatedly; phi-3-vision-128k/nvidia-vila/nvidia-neva-22b/
# cosmos-reason2-8b are all listed in /v1/models but 404 "Function ... Not
# found for account" - gated, need separate access approval on
# build.nvidia.com, not just an API key). nvidia/nemotron-3-nano-omni-30b-
# a3b-reasoning is the one that actually worked: real call against the real
# saved test frame returned a correctly-located box_2d for "black cube"
# (~3s, test_perception_zeroshot.py PASSes - see that script). Kept as the
# NVIDIA default; if this model later gets EOL'd/gated too, re-run the same
# /v1/models + real-call probe rather than guessing a new slug.
_NVIDIA_BASE_URL = "https://integrate.api.nvidia.com/v1/chat/completions"
_NVIDIA_MODEL_ID = "nvidia/nemotron-3-nano-omni-30b-a3b-reasoning"
_NVIDIA_API_KEY_FILE = os.path.expanduser("~/.nvidia_api_key")
_nvidia_key: str | None = None


def _nvidia_api_key() -> str:
    global _nvidia_key
    if _nvidia_key is None:
        key = os.environ.get("NVIDIA_API_KEY")
        if not key and os.path.exists(_NVIDIA_API_KEY_FILE):
            key = open(_NVIDIA_API_KEY_FILE).read().strip()
        if not key:
            raise RuntimeError(
                "No NVIDIA API key found - set NVIDIA_API_KEY or save one (from build.nvidia.com, "
                "starts with 'nvapi-') to " + _NVIDIA_API_KEY_FILE
            )
        _nvidia_key = key
    return _nvidia_key


def _call_nvidia(bgr_frame: np.ndarray, target_desc: str) -> list[tuple[str, float, tuple[int, int, int, int]]]:
    """Same contract as _call_gemini (see its docstring) - OpenAI-compatible
    chat/completions with an image_url data URI, same detect-prompt asking
    for 0-1000-normalized box_2d JSON (that instruction is spelled out in
    the prompt itself, not a Gemini-only convention, so it's reused as-is)."""
    ...
    resp = requests.post(
        _NVIDIA_BASE_URL,
        headers={"Authorization": f"Bearer {_nvidia_api_key()}", "Accept": "application/json"},
        json={
    ...
    if not resp.ok:
        print(f"[perception_zeroshot] NVIDIA API error {resp.status_code}: {resp.text[:200]}")
        return []
    ...
    for item in items:
        if not isinstance(item, dict):
            continue
        # 2026-09-02: this model names this key "box_2d" or "bbox_2d"
        # inconsistently call-to-call (confirmed live - same prompt, both
        # seen across consecutive requests) - only checking "box_2d" silently
        # dropped every "bbox_2d" response, showing as 0 detections.
        box = item.get("box_2d") or item.get("bbox_2d")
        if not (isinstance(box, list) and len(box) == 4):
            continue
        try:
            ymin, xmin, ymax, xmax = (float(v) for v in box)
...
# Selects which backend the 4 call sites below use - default stays "gemini"
# (the real-hardware-validated path) so adding NVIDIA doesn't change existing
# behavior; set PERCEPTION_BACKEND=nvidia to switch.
_BACKEND = os.environ.get("PERCEPTION_BACKEND", "gemini")


def _call_llm(bgr_frame: np.ndarray, target_desc: str) -> list[tuple[str, float, tuple[int, int, int, int]]]:
    return _call_nvidia(bgr_frame, target_desc) if _BACKEND == "nvidia" else _call_gemini(bgr_frame, target_desc)
...
def detect_zeroshot(bgr_frame: np.ndarray, text_prompt: str, box_threshold: float = BOX_THRESHOLD) -> Detection | None:
    ...
    docstring."""
    results = _call_llm(bgr_frame, text_prompt.strip().lower())
...
def detect_all_objects(bgr_frame: np.ndarray, candidate_labels: list[str] = DEFAULT_LABELS,
    ...
    results = _call_llm(bgr_frame, "all distinct physical objects on the table")
    out = []
