...
# 2026-09-01: that "no overlap" reasoning only needed interval_s to be
# nonzero, not 12 seconds - LabelWorker.run() already calls label_all_objects
# synchronously in a single loop, so passes can never overlap regardless of
# how short this wait is. The extra ~12s on TOP of every call's own latency
# (~3s NVIDIA backend / ~8-9s Gemini) was pure added staleness, not a real
# safety margin - per the user's "새로고침 딜레이... 최소화" request, lowered
# to keep just a small breather, not a real pause.
...
class LabelWorker(threading.Thread):
    ...
    def __init__(self, rgb_path: str = config.ASTRA_RGB_FRAME_PATH, interval_s: float = REFRESH_INTERVAL_S):
        ...
        self._latest_ts = 0.0
        # NOT named self._stop - threading.Thread already has its own internal
        # _stop() method, and shadowing it with an Event attribute crashes
        # Thread.join() ("'Event' object is not callable") the moment the
        # main loop exits normally (e.g. 'q'/ESC) and this worker gets
        # cleaned up - hit for real 2026-09-01, see git blame/session log.
