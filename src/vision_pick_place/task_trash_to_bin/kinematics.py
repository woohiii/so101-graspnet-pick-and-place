...
def solve_ik(kin: RobotKinematics, current_joint_deg: np.ndarray, target_xyz: tuple[float, float, float]) -> np.ndarray:
    ...
    # Preserve the current tool orientation while translating in XYZ.  The
    # previous top-down pose with orientation_weight=0 left the redundant IK
    # degrees of freedom unconstrained, allowing large wrist/elbow flips even
    # for millimetre-scale XY nudges.
    current_pose = kin.forward_kinematics(joints)
    target_pose = make_pose(target_xyz)
    target_pose[:3, :3] = current_pose[:3, :3]
    for _ in range(config.IK_ITERATIONS):
        joints = kin.inverse_kinematics(joints, target_pose, orientation_weight=0.1)
    return joints
